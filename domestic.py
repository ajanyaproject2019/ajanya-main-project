...
def send_report2():
    ...
    cmd.execute("insert into report_reply values(null,'"++"''" + title + "','"+file+"',curdate())")
    con.commit()
    return render_template("sendreport.html")
...
def human_report_view():
    cmd.execute("select * from admin_report where department='human rights' and status='pending'")
    con.commit()
    s=cmd.fetchall()
    print(s)
